...
p
#===============================================================================
df_t_human = df_t[df_t$Species=="Human",]
y_peak <- which.max(density(df_t_human$Ratio)$y)#找y值最大的
x_peak_human <- round(density(df_t_human$Ratio)$x[y_peak],3)#找出最大的y所对应的x,3)

df_t_yeast = df_t[df_t$Species=="Yeast",]
y_peak <- which.max(density(df_t_yeast$Ratio)$y)#找y值最大的
x_peak_yeast <- density(df_t_yeast$Ratio)$x[y_peak]#找出最大的y所对应的x 
x_peak_yeast <- round(x_peak_yeast,3)
df_t_ecoli = df_t[df_t$Species=="Ecoli",]
y_peak <- which.max(density(df_t_ecoli$Ratio)$y)#找y值最大的
x_peak_ecoli <- round(density(df_t_ecoli$Ratio)$x[y_peak],3)#找出最大的y所对应的x 
