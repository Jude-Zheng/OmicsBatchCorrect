...
library(data.table)
args <- commandArgs(T)
# args <- c("/home/zhl/projects/batch_effect_correction/new_results/ds.RData",
...
peptideinfo <- merge(Y_stand[,1],peptideinfo,by.x = "row.names",by.y="row.names",sort=F)
rownames(peptideinfo) <- peptideinfo[,1]
#peptideinfo_ <- peptideinfo$house
t_Y_stand <- t(Y_stand)
#RUVIII
ruviii <- RUVIII(t_Y_stand, M, ctl=peptideinfo$house, k=20)
...
# save
fwrite(ruviii, paste0(args[3],"protein_after_batch_RUVIII_.csv"))
print("RUVIII finish!!")
#############RUVIII_C
# library(RUVIIIC)
# #Y,M
...
#各个批次中蛋白的中位数除以所有蛋白中的中位数，得到每个蛋白在各个批次的因子
